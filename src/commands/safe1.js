...
exports.run = (database, message, args) => {
    if(message.author.id !== "933413214119669860") return message.react(config.emojis.red);
    ...
    if (!kullanıcı) return message.channel.send(noaEmbed.setDescription(`Güvenli listeye ekleyip çıkarmak istediğiniz kullanıcıyı belirtin. \nÖrnek: \`${config.prefix}${this.conf.name} @Noâ/ID\``).addField("Aktif güvenli kişiler listesi:", güvenli.length > 0 ? güvenli.map(noa => (message.guild.roles.cache.has(noa.slice(1)) || message.guild.members.cache.has(noa.slice(1))) ? (message.guild.roles.cache.get(noa.slice(1)) || message.guild.members.cache.get(noa.slice(1))) : noa).join('\n') : "`Listede kimse bulunmuyor.`")).then(e => message.react(config.emojis.notr));
