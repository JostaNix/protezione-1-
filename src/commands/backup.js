...
exports.run = async (database, message, args) => {
    moment.locale("tr")
    let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor(0x36041c).setTimestamp();
    ...
    if (argüman == "load" || argüman == "yükle" || argüman == "kur" || argüman == "oluştur") {
        ...
        if (kategori == "channel" || kategori == "kanal") {
            ...
            ChannelDatabase.findOne({ guildID: config.guildID, channelID: id }, async (err , channelData) => {
                ...
                await message.channel.send(noaEmbed.setDescription(`\`${channelData.name}\` adlı **kanal** yedeği kullanılarak **kanal** oluşturulacak. \nOnaylıyorsanız alttaki emojiye basın.`)).then(e => {
                    ...
                    collect.on("collect", async e => {
                        setTimeout(async function () {
                            ...
                            if (logChannel) logChannel.send(noaEmbed.setDescription(`${message.author} (\`${message.author.id}\`) tarafından \`${channelData.name}\` (\`${channelData.channelID}\`) **kanalının** yedeği kurulmaya başlandı.`))
                        }, 500);
                    })
                })
            })
        ...
        if (kategori == "role" || kategori == "rol") {
            ...
            RoleDatabase.findOne({ guildID: config.guildID, roleID: id }, async (err , roleData) => {
                ...
                await message.channel.send(noaEmbed.setDescription(`\`${roleData.name}\` adlı rolün yedeği kullanılarak rol oluşturulup, üyelere dağıtılacaktır. \nOnaylıyorsanız alttaki emojiye basın.`)).then(e => {
                    e.react(config.emojis.onay);
                    const onay = (reaction, member) => reaction.emoji.id === config.emojis.onay && member.id === message.author.id;
                    const collect = e.createReactionCollector(onay, { time: 60000 });
                    ...
                    collect.on("collect", async e => {
                        setTimeout(async function () {
                            ...
                            if (logChannel) logChannel.send(noaEmbed.setDescription(`${message.author} (\`${message.author.id}\`) tarafından \`${roleData.name}\` (\`${roleData.rolID}\`) **rolünün** yedeği kurulmaya başlandı. \nRol izinleri ayarlanıp bütün üyelere dağıtılacaktır.`))
                        }, 500);
                    })
                })
            })
