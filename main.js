...
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logkanal) logkanal.send(noaEmbed.setDescription(`${message.author} (\`${message.author.id}\`) kullanıcısı ${message.channel} (\`${message.channel.id}\`) kanalında \`${prefix}${cmd.conf.name}\` komutunu kullandı \nKomut İçeriği: \`${message.content}\` \n────────────────────────────────────────`))
}
});
...
guard1.on("guildMemberAdd", async noa => {
let guilty = await noa.guild.fetchAuditLogs({ type: "BOT_ADD" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.botAddGuard) return;
let logChannel = noa.guild.channels.cache.find(e => e.id === logs.guard1_log);
let kullanıcı = noa.guild.member(guilty.executor.id);
if (noa.user.bot == true) {
ytKapat(noa.guild.id);
if (kullanıcı.bannable) kullanıcı.ban({ reason: `${noa.user.username} | Noâ Bot Ekleme Koruması [Kullanıcı]` });
if (noa.bannable) noa.ban({ reason: `${kullanıcı.user.username} | Noâ Bot Ekleme Koruması [Bot]` });
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Bot Eklendi!** \n\n\`•\` **Botu Ekleyen Kullanıcı:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Eklenen Bot:** ${noa.user} \n\nBot ve botu ekleyen kullanıcı sunucudan ${kullanıcı.bannable ? "**başarıyla yasaklandı**" : "**yasaklanamadı**"}!`));
}
}
});

guard1.on("guildBanAdd", async (guild, noa) => {
...
ytKapat(guild.id);
if (kullanıcı.bannable) kullanıcı.ban({ reason: `${noa.username} | Noâ Sağ Tık Ban Koruması` });
guild.members.unban(noa.id, `${guilty.executor.username} Sağ Tık Ban Koruması`);
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Sağ Tık Ban Atıldı!** \n\n\`•\` **Sağ Tık Banlayan Yetkili:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Banlanan Kullanıcı:** ${noa.user} \n\nBanlanan kullanıcının banı açıldı ve sağ tık ban atan kullanıcı **başarıyla yasaklandı**!`));
}
});

guard1.on("guildMemberRemove", async noa => {
let guilty = await noa.guild.fetchAuditLogs({ type: "MEMBER_KICK" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.kickGuard) return;
let logChannel = noa.guild.channels.cache.find(e => e.id === logs.guard1_log);
let kullanıcı = noa.guild.member(guilty.executor.id);
ytKapat(noa.guild.id);
if (kullanıcı.bannable) kullanıcı.ban({ reason: `${noa.user.username} | Noâ Sağ Tık Koruması` });
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Sağ Tık Kick Atıldı!** \n\n\`•\` **Sağ Tık Kickleyen Yetkili:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Kicklenen Kullanıcı:** ${noa.user} \n\nSağ tık kick atan kullanıcı sunucudan ${kullanıcı.bannable ? "**başarıyla yasaklandı**" : "**yasaklanamadı**"}!`));
}
});

guard1.on("guildMemberUpdate", async (eskinoa, yeninoa) => {
if (yeninoa.roles.cache.size > eskinoa.roles.cache.size) {
let guilty = await yeninoa.guild.fetchAuditLogs({ type: "MEMBER_ROLE_UPDATE" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.memberGuard) return;
let logChannel = yeninoa.guild.channels.cache.find(e => e.id === logs.guard1_log);
let kullanıcı = yeninoa.guild.member(guilty.executor.id);
if (perms.some(e => !eskinoa.hasPermission(e) && yeninoa.hasPermission(e))) {
if (kullanıcı.bannable) kullanıcı.ban({ reason: `Noâ Sağ Tık Rol Verme Koruması` });
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Sağ Tık Rol Verildi/Alındı!** \n\n\`•\` **Sağ Tık Rol Veren/Alan Kullanıcı:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Rol Verilen/Alınan Kullanıcı:** ${yeninoa.user} (\`${yeninoa.user.id}\`) \n\nSağ tık rol alıp/veren kullanıcı sunucudan ${kullanıcı.bannable ? "**başarıyla yasaklandı**" : "**yasaklanamadı**"}!`));
}
}
}
});

guard1.on("roleCreate", async noa => {
let guilty = await noa.guild.fetchAuditLogs({ type: "ROLE_CREATE" }).then(e => e.entries.first());
...
noa.delete({ reason: `Noâ Rol Oluşturma Koruması` });
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Rol Oluşturuldu!** \n\n\`•\` **Rol Oluşturan Kullanıcı:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\n\`•\` **Oluşturulan Rol Bilgileri:** ${noa.name} (\`${noa.id}\`) \n\nRol oluşturan kullanıcı ${kullanıcı.manageable ? "**başarıyla cezalandırıldı**" : "**cezalandırılamadı**"}!`));
}
});

guard1.on("roleUpdate", async (eskinoa, yeninoa) => {
let guilty = await yeninoa.guild.fetchAuditLogs({ type: "ROLE_UPDATE" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.roleGuard) return;
let logChannel = yeninoa.guild.channels.cache.find(e => e.id === logs.guard1_log);
let kullanıcı = yeninoa.guild.member(guilty.executor.id);
ytKapat(yeninoa.guild.id);
if (kullanıcı.manageable) kullanıcı.roles.cache.has(config.booster) ? kullanıcı.roles.set([config.booster, config.jail]) : kullanıcı.roles.set([config.jail])
if (perms.some(e => !eskinoa.permissions.has(e) && yeninoa.permissions.has(e))) {
yeninoa.setPermissions(eskinoa.permissions);
yeninoa.guild.roles.cache.filter(e => !e.managed && (e.permissions.has("ADMINISTRATOR") || e.permissions.has("MANAGE_GUILD") || e.permissions.has("MANAGE_ROLES") || e.permissions.has("MANAGE_CHANNELS") || e.permissions.has("MANAGE_EMOJIS") || e.permissions.has("MANAGE_NICKNAMES") || e.permissions.has("MANAGE_WEBHOOKS"))).forEach(e => e.setPermissions(36818497));
};
yeninoa.edit({
name: eskinoa.name,
color: eskinoa.hexColor,
hoist: eskinoa.hoist,
permissions: eskinoa.permissions,
mentionable: eskinoa.mentionable
});
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.sendnoaEmbed.setDescription((`**Rol Güncellendi!** \n\n\`•\` **Rolü Güncelleyen Kullanıcı:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Güncellenen Rol:** \`${yeninoa.name}\` (\`${yeninoa.id}\`) \n\nRolü güncelleyen kullanıcı sunucudan ${kullanıcı.manageable ? "**başarıyla cezalandırıldı**" : "**cezalandırılamadı**"}!`));
}
});

guard1.on("roleDelete", async noa => {
let guilty = await noa.guild.fetchAuditLogs({ type: "ROLE_DELETE" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.roleGuard) return;
let logChannel = noa.guild.channels.cache.find(e => e.id === logs.guard1_log);
let kullanıcı = noa.guild.member(guilty.executor.id);
if (kullanıcı.bannable) kullanıcı.ban({ reason: `Noâ Rol Silme Koruması` });
ytKapat(noa.guild.id);
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Rol Silindi!** \n\n\`•\` **Rolü Silen Kullanıcı:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Silinen Rol:** \`${noa.name}\` (\`${noa.id}\`) \n\nRolü silen kullanıcı sunucudan ${kullanıcı.bannable ? "**başarıyla yasaklandı**" : "**yasaklanamadı**"}!`));
}
});
...
if (kullanıcı.bannable) kullanıcı.ban({ reason: "Noâ Kanal Güncelleme Koruması" });
...
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Kanal Güncellendi!** \n\n\`•\` **Kanalı Güncelleyen Yetkili:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Güncellenen Kanal:** \`${newChannel.name}\` (\`${newChannel.id}\`) \n\nKanalı güncelleyen kullanıcı ${kullanıcı.bannable ? "**başarıyla yasaklandı**" : "**yasaklanamadı**"}!`));
}
});

guard2.on("channelDelete", async noa => {
let guilty = await noa.guild.fetchAuditLogs({ type: "CHANNEL_DELETE" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.channelGuard) return;
let logChannel = noa.guild.channels.cache.find(e => e.id === logs.guard2_log);
let kullanıcı = noa.guild.member(guilty.executor.id);
ytKapat(noa.guild.id);
if (kullanıcı.bannable) kullanıcı.ban({ reason: "noa Kanal Silme Koruması" });
const newChannel = await channel.clone({ reason: "noa Kanal Silme Koruması"});
if(channel.tpye == "category") channel.children.forEach(x => x.setParent(newChannel.id))
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Kanal Silindi!** \n\n\`•\` **Kanalı Silen Yetkili:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Silinen Kanal:** \`${noa.name}\` (\`${noa.id}\`) \n\nKanalı silen kullanıcı ${kullanıcı.bannable ? "**başarıyla yasaklandı**" : "**yasaklanamadı**"}!`));
}
});

guard2.on("guildUpdate", async (eskinoa, yeninoa) => {
let guilty = await yeninoa.fetchAuditLogs({ type: "GUILD_UPDATE" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now() - guilty.createdTimestamp > 10000 || safe1(guilty.executor.id) || !guards.urlGuard) return;
let logChannel = yeninoa.channels.cache.find(e => e.id === logs.guard2_log);
let kullanıcı = yeninoa.member(guilty.executor.id);
if (eskinoa.vanityURLCode !== yeninoa.vanityURLCode) {
if (kullanıcı.bannable) kullanıcı.ban({ reason: `noa Özel URL Koruması` });
const ayarlar = {
url: `https://discord.com/api/v6/guilds/${yeninoa.id}/vanity-url`,
body: {
code: config.url
},
json: true,
...
guard3.on("webhookUpdate", async noa => {
let guilty = await noa.guild.fetchAuditLogs({ type: "WEBHOOK_CREATE" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.webHookGuard) return;
let logChannel = noa.guild.channels.cache.find(e => e.id === logs.guard3_log);
let kullanıcı = noa.guild.member(guilty.executor.id);
ytKapat(noa.guild.id);
if (kullanıcı.bannable) kullanıcı.ban({ reason: "noa Webhook Koruması" });
const webhooklar = await noa.fetchWebhooks();
await webhooklar.map(e => e.delete({ reason: `noa Webhook Koruması` }));
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Webhook Oluşturuldu!** \n\n\`•\` **Webhook Oluşturan Yetkili:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Webhook Oluşturulan Kanal:** ${noa} (\`${noa.id}\`) \n\nWebhook oluşturan kullanıcı ${kullanıcı.bannable ? "**başarıyla yasaklandı**" : "**yasaklanamadı**"}!`));
}
});

guard3.on("emojiCreate", async noa => {
let guilty = await noa.guild.fetchAuditLogs({ type: "EMOJI_CREATE" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.emojiGuard) return;
let logChannel = noa.guild.channels.cache.find(e => e.id === logs.guard3_log);
let kullanıcı = noa.guild.member(guilty.executor.id)
if (kullanıcı.manageable) kullanıcı.roles.cache.has(config.booster) ? kullanıcı.roles.set([config.booster, config.jail]) : kullanıcı.roles.set([config.jail])
noa.delete({ reason: "noa Emoji Yükleme Koruması" });
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Emoji Oluşturuldu!** \n\n\`•\` **Emoji Oluşturan Yetkili:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Oluşturulan Emoji:** \`${noa.name}\` (\`${noa.id}\`) \n\nEmojiyi yükleyen kullanıcı ${kullanıcı.manageable ? "**başarıyla cezalandırıldı**" : "**cezalandırılamadı**"}!`))
}
});

guard3.on("emojiUpdate", async (eskinoa, yeninoa) => {
let guilty = await yeninoa.guild.fetchAuditLogs({ type: "EMOJI_UPDATE" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.emojiGuard) return;
let logChannel = yeninoa.guild.channels.cache.find(e => e.id === logs.guard3_log);
let kullanıcı = yeninoa.guild.member(guilty.executor.id)
if (kullanıcı.manageable) kullanıcı.roles.cache.has(config.booster) ? kullanıcı.roles.set([config.booster, config.jail]) : kullanıcı.roles.set([config.jail])
if (yeninoa.name !== eskinoa.name) yeninoa.setName(eskinoa.name);
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Emoji Güncellendi!** \n\n\`•\` **Emoji Oluşturan Yetkili:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Emojinin Eski Adı:** \`${eskinoa.name}\` \n\`•\` **Emojinin Yeni Adı:** \`${yeninoa.name}\` \n\`•\` **Emojinin ID'si:** \`${yeninoa.id}\` \n\nEmojiyi güncelleyen kullanıcı ${kullanıcı.manageable ? "**başarıyla cezalandırıldı**" : "**cezalandırılamadı**"}!`));
}
});

guard3.on("emojiDelete", async noa => {
let guilty = await noa.guild.fetchAuditLogs({ type: "EMOJI_DELETE" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.emojiGuard) return;
let logChannel = noa.guild.channels.cache.find(e => e.id === logs.guard3_log);
let kullanıcı = noa.guild.member(guilty.executor.id)
if (kullanıcı.manageable) kullanıcı.roles.cache.has(config.booster) ? kullanıcı.roles.set([config.booster, config.jail]) : kullanıcı.roles.set([config.jail])
noa.guild.emojis.create(noa.url, noa.name);
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Emoji Silindi!** \n\n\`•\` **Emoji Silen Yetkili:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\`•\` **Silinen Emoji:** \`${noa.name}\` (\`${noa.id}\`) \n\nEmojiyi silen kullanıcı ${kullanıcı.manageable ? "**başarıyla cezalandırıldı**" : "**cezalandırılamadı**"}!`));
}
});

guard3.on("guildUpdate", async (eskinoa, yeninoa) => {
let guilty = await yeninoa.fetchAuditLogs({ type: "GUILD_UPDATE" }).then(e => e.entries.first());
if (!guilty || !guilty.executor || Date.now()-guilty.createdTimestamp > 5000 || safe1(guilty.executor.id) || !guards.serverGuard) return;
let logChannel = yeninoa.guild.channels.cache.find(e => e.id === logs.guard3_log);
let kullanıcı = yeninoa.guild.member(guilty.executor.id);
if (kullanıcı.bannable) kullanıcı.ban({ reason: `Noâ Sunucu Güncelleme Koruması` });
if (yeninoa.name !== eskinoa.name) yeninoa.setName(eskinoa.name);
if (yeninoa.iconURL({ dynamic: true }) !== eskinoa.iconURL({ dynamic: true })) yeninoa.setIcon(eskinoa.iconURL({ dynamic: true }));
if (yeninoa.afkChannel !== eskinoa.afkChannel) yeninoa.setAFKChannel(eskinoa.afkChannel);
if (yeninoa.banner !== eskinoa.banner) yeninoa.setBanner(eskinoa.bannerURL);
if (yeninoa.region !== eskinoa.region) yeninoa.setRegion(eskinoa.region);
let noaEmbed = new Discord.MessageEmbed().setFooter(config.footer, database.guilds.cache.get(config.guildID).iconURL({ dynamic: true })).setColor("RANDOM").setTimestamp();
if (logChannel) {
logChannel.send(noaEmbed.setDescription(`**Sunucu Güncellendi!** \n\n\`•\` **Sunucuyu Güncelleyen Kişi:** ${guilty.executor} (\`${guilty.executor.id}\`) \n\nSunucuyu güncelleyen kullanıcı ${kullanıcı.bannable ? "**başarıyla yasaklandı**" : "**yasaklanamadı**"}!`));
}
});
